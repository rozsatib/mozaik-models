...
def perform_analysis_and_visualization_stc(data_store):
    ...
    for sheet in data_store.sheets():
        if eval(data_store.full_datastore.block.annotations['sheet_parameters'])[sheet]['cell']['model'] in ["aeif_cond_alpha_multisynapse","aeif_cond_beta_multisynapse"]:
            dsv = param_filter_query(data_store, sheet_name = sheet)
            ExcitatoryConductanceGenerator(dsv, ParameterSet({'excitatory_receptors': ['AMPA', 'NMDA'], 'replace_if_exists':True})).analyse()
            InhibitoryConductanceGenerator(dsv, ParameterSet({'inhibitory_receptors': ['GABAA','GABAB'], 'replace_if_exists':True})).analyse()
...
def perform_analysis_and_visualization(data_store):
    ...
    if True:  # PLOTTING
        ...
        if paper_figures:
            ...
            PerNeuronValuePlot(dsv,ParameterSet({"cortical_view" : True, "neuron_ids": spike_ids}),plot_file_name='ORSet.png').plot()
