...
class MRfigReal(Plotting):
    ...
    def plot(self):
        ...
        ax.hist(l4_v_mr, bins=numpy.arange(
            0, f1_f0_vm_xmax+0.01, 0.3), color='gray', rwidth=0.8)
        disable_top_right_axis(ax)
        disable_left_axis(ax)
        disable_xticks(ax)
        remove_x_tick_labels()
        remove_y_tick_labels()
        pylab.xlim(0, f1_f0_vm_xmax)
        ax.title.set_text('Mean F1/F0 Vm: %.3f' % np.mean(l4_v_mr))

        if self.parameters.ComplexSheetName != 'None':
            ax = pylab.subplot(gs[1, 1])
            ax.hist(l23_v_mr, bins=numpy.arange(
                0, f1_f0_vm_xmax+0.01, 0.3), color='gray', rwidth=0.8)
            ax.title.set_text('Mean F1/F0 Vm: %.3f' % np.mean(l23_v_mr))
            disable_top_right_axis(ax)
            disable_left_axis(ax)
            disable_xticks(ax)
            remove_x_tick_labels()
            ...
            ax.hist([complex_v_mr, simple_v_mr], bins=numpy.arange(
                0, f1_f0_vm_xmax+0.01, 0.3), histtype='barstacked', color=['w', 'k'], rwidth=0.8, ec='black')
            three_tick_axis(ax.xaxis)
            remove_y_tick_labels()
        ...
        ax.hist(dsv_l4_v_F0[0].get_value_by_id(l4_ids), bins=numpy.arange(
            f0_vm_xmin, f0_vm_xmax + 0.01, 0.4), color='gray', rwidth=0.8)
        disable_top_right_axis(ax)
        disable_left_axis(ax)
        disable_left_axis(ax)
        disable_xticks(ax)
        remove_x_tick_labels()
        remove_y_tick_labels()
        ax.title.set_text('Mean F0 Vm: %.3f' % np.mean(dsv_l4_v_F0[0].get_value_by_id(l4_ids)))
        pylab.xlim(f0_vm_xmin, f0_vm_xmax)

        if self.parameters.ComplexSheetName != 'None':
            ax = pylab.subplot(gs[1, 2])
            ax.hist(dsv_l23_v_F0[0].get_value_by_id(l23_ids), bins=numpy.arange(
                f0_vm_xmin, f0_vm_xmax + 0.01, 0.4), color='gray', rwidth=0.8)
            ax.title.set_text('Mean F0 Vm: %.3f' % np.mean(dsv_l23_v_F0[0].get_value_by_id(l23_ids)))
            disable_top_right_axis(ax)
            disable_left_axis(ax)
            disable_xticks(ax)
            remove_x_tick_labels()
            ...
            ax.hist([dsv_complex_v_F0, dsv_simple_v_F0], bins=numpy.arange(
                f0_vm_xmin, f0_vm_xmax + 0.01, 0.4), histtype='barstacked', color=['w', 'k'], rwidth=0.8, ec='black')
            three_tick_axis(ax.xaxis)
            remove_y_tick_labels()
        ...
        ax.hist(dsv_l4_v_F1[0].get_value_by_id(l4_ids), bins=numpy.arange(
            0, f1_vm_xmax + 0.01, 0.5), color='gray', rwidth=0.8)
        disable_top_right_axis(ax)
        disable_left_axis(ax)
        disable_xticks(ax)
        remove_x_tick_labels()
        remove_y_tick_labels()
        pylab.xlim(0, f1_vm_xmax)
        ax.title.set_text('Mean F1 Vm: %.3f' % np.mean(dsv_l4_v_F1[0].get_value_by_id(l4_ids)))

        if self.parameters.ComplexSheetName != 'None':
            ax = pylab.subplot(gs[1, 3])
            ax.hist(dsv_l23_v_F1[0].get_value_by_id(l23_ids), bins=numpy.arange(
                0, f1_vm_xmax + 0.01, 0.5), color='gray', rwidth=0.8)
            ax.title.set_text('Mean F1 Vm: %.3f' % np.mean(dsv_l23_v_F1[0].get_value_by_id(l23_ids)))
            disable_top_right_axis(ax)
            disable_left_axis(ax)
            disable_xticks(ax)
            remove_x_tick_labels()
            ...
            ax.hist([dsv_complex_v_F1, dsv_simple_v_F1], bins=numpy.arange(
                0, f1_vm_xmax + 0.01, 0.5), histtype='barstacked', color=['w', 'k'], rwidth=0.8, ec='black')
            three_tick_axis(ax.xaxis)
            remove_y_tick_labels()
